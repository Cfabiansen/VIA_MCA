...
subject = {'003','006','007','009','026','027','028','029','038','041','043','044', ...
    '046','070','078','082','090','094','098','101','102','123','127','130','131', ...
    '137','142','146','162','164','182','183','196','242','279','373','388','381','387'}; % all controls

for j = 1:numel(subject)
load(['/mnt/projects/VIA11/EEG/Data/###_Flanker/EEG_analysis/0' subject{j} '_data.mat'])
load(['/mnt/projects/VIA11/EEG/Data/###_Flanker/EEG_analysis/0' subject{j} '_info.mat'])
...
timelock_con_R(j).LRP = ft_timelockanalysis(cfg,data_con); % trials averaging

timelock_con_R(j).LRP.avg(1:128,:) = ft_preproc_rereference(timelock_con_R(j).LRP.avg(1:128,:)); % average referencing

cfg = [];
cfg.baseline = [-0.5 -0.1];
timelock_con_R(j).LRP = ft_timelockbaseline(cfg, timelock_con_R(j).LRP); % baseline correction


cfg = [];
cfg.trials = trials_incon_R;
timelock_incon_R(j).LRP = ft_timelockanalysis(cfg,data_incon); % trials averaging

timelock_incon_R(j).LRP.avg(1:128,:) = ft_preproc_rereference(timelock_incon_R(j).LRP.avg(1:128,:)); % average referencing

cfg = [];
cfg.baseline = [-0.5 -0.1];
timelock_incon_R(j).LRP = ft_timelockbaseline(cfg, timelock_incon_R(j).LRP); % baseline correction

cfg = [];
cfg.trials = trials_con_L;
timelock_con_L(j).LRP = ft_timelockanalysis(cfg,data_con); % trials averaging

timelock_con_L(j).LRP.avg(1:128,:) = ft_preproc_rereference(timelock_con_L(j).LRP.avg(1:128,:)); % average referencing

cfg = [];
cfg.baseline = [-0.5 -0.1];
timelock_con_L(j).LRP = ft_timelockbaseline(cfg, timelock_con_L(j).LRP); % baseline correction


cfg = [];
cfg.trials = trials_incon_L;
timelock_incon_L(j).LRP = ft_timelockanalysis(cfg,data_incon); % trials averaging

timelock_incon_L(j).LRP.avg(1:128,:) = ft_preproc_rereference(timelock_incon_L(j).LRP.avg(1:128,:)); % average referencing

cfg = [];
cfg.baseline = [-0.5 -0.1];
timelock_incon_L(j).LRP = ft_timelockbaseline(cfg, timelock_incon_L(j).LRP); % baseline correction


cfg = [];
cfg.channelcmb = {'D19' 'B22'};    % D19 = C3 = electrode 115 and B22 = C4 = electrode 54

LRP_con_K(i).LRP = ft_lateralizedpotential(cfg,timelock_con_R(i).LRP,timelock_con_L(i).LRP); %TURNED AROUND

LRP_incon_K(i).LRP = ft_lateralizedpotential(cfg,timelock_incon_R(i).LRP,timelock_incon_L(i).LRP); %TURNED AROUND
...
save(['/mnt/projects/VIA11/EEG/Anna/Flanker_LRP/PBC_LRP_data.mat'], 'subject_K', 'timelock_con_L_K', 'timelock_con_R_K', 'timelock_incon_L_K', 'grandavg_con_LRP_K', 'grandavg_incon_LRP_K', 'LRP_con_K', 'LRP_incon_K');
...
subject_SZ = {'020','030','031','036','048','049','063','064','068','069','072','083', ...
    ...
    '368','369','409','312','354','384'};


for j = 1:numel(subject_SZ)
load(['/mnt/projects/VIA11/EEG/Data/###_Flanker/EEG_analysis/0' subject_SZ{j} '_data.mat'])
load(['/mnt/projects/VIA11/EEG/Data/###_Flanker/EEG_analysis/0' subject_SZ{j} '_info.mat'])
...
%% Mean amplitude calculation for timewindow 150-849 ms
