...
##########################################################################
############ VIA15 ACCURACY REACTION TIME TRADE OFF ANALYSIS #############
##########################################################################
...
fig1<-ggplot(summary_data, aes(x = MeanRT, y = MeanAccuracy, group = hgr_status, color = hgr_status)) +
  ...
  labs(title = "Congruent accuracy",
       x = "Reaction Time [ms]",
       y = "Accuracy [%]") +
  scale_color_manual(values = c("FHR_BP" = "#8cd0db", "FHR_SZ" = "#00afc2", "PBC" = "#B3D235")) # Customize colors as needed
...
######### VIA15 CONGRUENCY EFFECT ACCURACY ########

VIA15_data <- read_excel("/mnt/projects/VIA_MCA/nobackup/Clean_VIA15_Flanker_23_04.xlsx")

VIA15_data$mean_RT = rowMeans(VIA15_data[, c("RT_CC", "RT_IC")], na.rm = TRUE)
