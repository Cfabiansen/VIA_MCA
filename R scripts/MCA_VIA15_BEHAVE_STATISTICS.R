...
################################################
######### VIA 15 FHR VS PBS (2 GROUPS) #########
################################################

# Load data
library(readxl)
getwd
data <- read_excel("/mnt/projects/VIA_MCA/nobackup/Clean_VIA15_Flanker_23_04.xlsx")
...
################################################
####### VIA 15 HGR VS PBS (ALL 3 GROUPS) #######
################################################
...
violin_VIA15_RT <- ggplot(data, aes(x = hgr_status, y = RT, fill = condition_status)) +  # Use combined variable for fill
  geom_violin(trim = FALSE, scale = "width") +
  geom_dotplot(binaxis = "y", stackdir = "center", stackratio = 1, dotsize = 0.5, 
               position = position_dodge(width = 0.9)) +
  labs(title = "VIA15 Reaction time distribution",x = "Status", y = "Reaction time") +
  theme_bw() +
...
violin_VIA15_ACC <- ggplot(data, aes(x = hgr_status, y = ACC, fill = condition_status)) +  # Use combined variable for fill
  geom_violin(trim = FALSE, scale = "width") +
  geom_dotplot(binaxis = "y", stackdir = "center", stackratio = 1, dotsize = 0.5, 
               position = position_dodge(width = 0.9)) +
  labs("VIA15 Accuracy distribution", x = "Status", y = "Accuracy") +
  theme_bw() +
  scale_fill_manual(values = custom_colors)   # Apply custom colors based on combined condition and status
